...
def load_imdb_memorable(dbname):
  ...
  pos = MFile.pickle_load_mac('../data/pn2/poslinesdict2.newmatch.111511.pickle')
  pos2 = {}
...
def load_imdb_memorable_results(dbname):
  merge_db = MDb.MMySQLDb(dbname)
  mdb = MDb.MDb('../data/pn2/db/poslinesdict.newdata.bing.sqlite')
...
def add_memorable_quotes_to_main_table(dbname):
  ...
  merge_db = MDb.MMySQLDb(dbname)
  ids = merge_db.q("SELECT q.id, m.imdb_quote FROM quotes as q, memorable as m WHERE q.is_memorable = 1 AND q.quote=m.matched_quote AND q.movie_name=m.movie_name")
  # print ids
  for qid, imdb_quote in ids:
    merge_db.qt("UPDATE quotes SET imdb_quote=%s WHERE id=%s", (imdb_quote, qid))
  merge_db.commit()
  merge_db.close()
...
# load_imdb_memorable_results(DBNAME) # TODO
# load_movies(DBNAME)
# label_memorable_quotes(DBNAME)
# add_memorable_quotes_to_main_table(DBNAME) # TODO
...
# populate_fixed_results(DBNAME, 'memorable', 'imdb_quote') # TODO
# generate_csv_from_db(DBNAME, '../data/merged/posneg2.txt') # TODO
