...
for movie in valid_movies:
  ...
  db.qt('INSERT INTO movies (movie_name, is_imdb_top) VALUES(%s, %s)', (movie, is_imdb_top))
  
  # insert into quote_pairs
  i = 0
  for pos, neglist in lpairs[movie]:
    if i < 2:
      db.qt('INSERT INTO quote_pairs (group_id, movie_name, movie_title, quote_1, quote_2) VALUES(%s, %s, %s, %s, %s)', (1, movie, movie.title(), pos[4], neglist[0][4]))
      i += 1  
  
  # insert into quote_short_term
  pos, neglist = spairs[movie][0]
  db.qt('INSERT INTO quote_short_term (movie_name, movie_title, quote_type, quote) VALUES(%s, %s, %s, %s)', (movie, movie.title(), 'positive', pos[4]))
  db.qt('INSERT INTO quote_short_term (movie_name, movie_title, quote_type, quote) VALUES(%s, %s, %s, %s)', (movie, movie.title(), 'negative', neglist[0][4]))
  for i in range(1,5):
    db.qt('INSERT INTO quote_short_term (movie_name, movie_title, quote_type, quote) VALUES(%s, %s, %s, %s)', (movie, movie.title(), 'negative_rest', neglist[i][4]))
